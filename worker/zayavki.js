...
// Номер версии этого файла. Виден снаружи: если открыть адрес Worker
// в браузере, он будет в ответе. Так сразу понятно, какой код реально
// работает — а то на глаз старая и новая версия неотличимы.
// Меняете код — увеличьте номер.
const VERSION = '1.2-lang-en';
  ...
  async fetch(request, env) {
    ...
    if (request.method !== 'POST') {
      return answer({
        ok: false,
        version: VERSION,
        error: 'Сюда нужно отправлять заявку, а не открывать в браузере'
      }, 405, cors);
    }
